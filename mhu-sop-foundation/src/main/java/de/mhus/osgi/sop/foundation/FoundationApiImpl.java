...
public class FoundationApiImpl extends MLog implements FoundationApi {
    ...
    private static final SearchHelper SEARCH_HELPER_FOUNDATION =
            new SearchHelper() {
                @Override
                public String findKeyForValue(AQuery<?> query, String value) {
                    if (MValidator.isUUID(value)) return "id";
                    return "ident";
                }
            };

    private static final SearchHelper SEARCH_HELPER_DATA =
            new SearchHelper() {
                @Override
                public String findKeyForValue(AQuery<?> query, String value) {
                    if (MValidator.isUUID(value)) return "id";
    ...
    @Override
    public SopJournal appendJournalEntry(
            UUID foundation, String queue, String event, String... data) throws MException {
        SopJournal item =
                getManager()
                        .inject(new SopJournal(foundation, queue, event, getJournalOrder(), data));
        item.save();
        return item;
    }
    ...
    @Override
    public SopJournal appendJournalEntry(
            UUID foundation, String queue, String event, Map<String, Object> data)
            throws MException {
        SopJournal item =
                getManager()
                        .inject(new SopJournal(foundation, queue, event, getJournalOrder(), data));
    ...
    public SopJournal getJournalEntry(String id) throws MException {
        ...
        if (order > 0)
            return getManager()
                    .getObjectByQualification(Db.query(SopJournal.class).eq("order", order));
    ...
    @Override
    public List<SopJournal> getJournalEntries(
            UUID foundation, String queue, long since, int max, int page, String search)
            throws MException {

        AQuery<SopJournal> query =
                Db.query(SopJournal.class).eq("queue", queue).eq("foundation", foundation);
        if (since > 0) query.gt(_SopJournal._ORDER, since);
    ...
    public SopFoundationGroup getFoundationGroup(String group) throws MException {
        return getManager()
                .getObjectByQualification(Db.query(SopFoundationGroup.class).eq("name", group));
    }
    ...
    public SopDataController getDataSyncControllerForType(String type) {
        try {
            SopDataController out =
                    MOsgi.getService(SopDataController.class, "(type=" + type + ")");
            return out;
    ...
    @Override
    public List<SopData> getSopData(
            UUID foundId,
            String type,
            String search,
        ...
        if (type != null) query.eq(_SopData._TYPE, type);
        if (foundId != null) query.eq(_SopData._FOUNDATION, foundId);
        if (publicAccess)
            //			query.eq(Db.attr("ispublic"), Db.value(true));
            query.eq(_SopData._IS_PUBLIC, true);
        if (due != null) query.lt(_SopData._DUE, due);
        ...
        if (MString.isSet(search)) {
            for (String part : search.split(" ")) {
                ...
                if (MString.isIndex(part, ':')) {
                    ...
                    switch (key) {
                        case "value0":
                            query.eq(_SopData._VALUE0, val);
                            break;
                        case "value1":
                            query.eq(_SopData._VALUE1, val);
                            break;
                        case "value2":
                            query.eq(_SopData._VALUE2, val);
                            break;
                        case "value3":
                            query.eq(_SopData._VALUE3, val);
                            break;
                        case "value4":
                            query.eq(_SopData._VALUE4, val);
                            break;
                        case "value5":
                            query.eq(_SopData._VALUE5, val);
                            break;
                        case "value6":
                            query.eq(_SopData._VALUE6, val);
                            break;
                        case "value7":
                            query.eq(_SopData._VALUE7, val);
                            break;
                        case "value8":
                            query.eq(_SopData._VALUE8, val);
                            break;
                        case "value9":
                            query.eq(_SopData._VALUE9, val);
                            break;
                        case "*value0*":
                            query.like(_SopData._VALUE0, "%" + val + "%");
                            break;
                        case "*value1*":
                            query.like(_SopData._VALUE1, "%" + val + "%");
                            break;
                        case "*value2*":
                            query.like(_SopData._VALUE2, "%" + val + "%");
                            break;
                        case "*value3*":
                            query.like(_SopData._VALUE3, "%" + val + "%");
                            break;
                        case "*value4*":
                            query.like(_SopData._VALUE4, "%" + val + "%");
                            break;
                        case "*value5*":
                            query.like(_SopData._VALUE5, "%" + val + "%");
                            break;
                        case "*value6*":
                            query.like(_SopData._VALUE6, "%" + val + "%");
                            break;
                        case "*value7*":
                            query.like(_SopData._VALUE7, "%" + val + "%");
                            break;
                        case "*value8*":
                            query.like(_SopData._VALUE8, "%" + val + "%");
                            break;
                        case "*value9*":
                            query.like(_SopData._VALUE9, "%" + val + "%");
                            break;
                        case "writable":
                            query.eq(_SopData._IS_WRITABLE, MCast.toboolean(val, false));
                            break;
                        case "archived":
                            isArchived = true;
                            if (!"all".equals(val))
        ...
        if (!isArchived)
            if (archived != null) query.eq(_SopData._ARCHIVED, archived.booleanValue());

        if (order == null) query.desc("foreignid");
        else {
            boolean asc = true;
            ...
            if (asc) query.asc(order);
            else query.desc(order);
        }
    ...
    public SopData getSopData(UUID foundId, UUID id, boolean sync) throws MException {
        ...
        if (foundId != null && !out.getFoundation().equals(foundId)) return null;

        if (sync) syncSopData(out, false, true);
                    ...
                    getManager()
                            .getObjectByQualification(
                                    Db.extendObjectQueryFromSearch(
                                            Db.query(SopData.class), id, SEARCH_HELPER_DATA));
        }
        if (out == null) return null;
        if (foundId != null && !out.getFoundation().equals(foundId)) return null;
    ...
    public boolean syncSopData(SopData obj, boolean forced, boolean save) {
        ...
        if (!forced) {
            if (!sync.isNeedSync(obj)) return false;
        }
    ...
    public List<SopFoundation> searchFoundations(String search, int page) throws MException {
        ...
        if (!context.isAdminMode()) {
            List<UUID> list =
                    FoundationContextCacheService.get(
                            context, SopFoundation.class.getCanonicalName() + "_" + search);
            if (list != null) {
        ...
        LinkedList<SopFoundation> out =
                RestUtil.collectResults(
                        getManager(),
                        Db.extendObjectQueryFromSearch(
        ...
        if (!context.isAdminMode() && out.size() < RestUtil.MAX_RETURN_SIZE) {
            ...
            for (SopFoundation o : out) cached.add(o.getId());

            FoundationContextCacheService.set(
                    context,
                    SopFoundation.class.getCanonicalName() + "_" + search,
                    MPeriod.HOUR_IN_MILLISECOUNDS,
                    cached);
        }
    ...
    public SopFoundation getFoundation(String id) throws MException {
        ...
        return getManager()
                .getObjectByQualification(
                        Db.extendObjectQueryFromSearch(
                                Db.query(SopFoundation.class), id, SEARCH_HELPER_FOUNDATION));
    }
}
