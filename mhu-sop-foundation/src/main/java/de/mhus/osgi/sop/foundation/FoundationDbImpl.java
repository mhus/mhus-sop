...
public class FoundationDbImpl extends AbstractDbSchemaService {
    ...
    public void doPostInitialize(XdbService manager) throws Exception {
        ...
        try (AaaContext ctx = AaaUtil.enterRoot()) {
            // init base structure
            SopFoundationGroup defGroup =
                    service.getObjectByQualification(
                            Db.query(SopFoundationGroup.class).eq(_SopFoundationGroup._NAME, ""));
            if (defGroup == null) {
                defGroup = service.inject(new SopFoundationGroup(""));
                defGroup.save();
            }

            SopFoundation defFound =
                    service.getObjectByQualification(
                            Db.query(SopFoundation.class).eq(_SopFoundation._IDENT, ""));
            if (defFound == null) {
                ...
                SopAcl acl =
                        service.getObjectByQualification(
                                Db.query(SopAcl.class)
                                        .eq(_SopAcl._TARGET, defFoundationId.toString()));
                if (acl == null) {
            ...
            try {
                SopAcl acl =
                        service.getObjectByQualification(
                                Db.query(SopAcl.class).eq(_SopAcl._TARGET, defFoundationId + "_"));
                if (acl == null) {
    ...
    public void collectReferences(Persistable object, ReferenceCollector collector) {
        ...
        try {
            for (SopObjectParameter p :
                    M.l(AdbApi.class).getParameters(object.getClass(), meta.getId())) {
    ...
    public String getAcl(AaaContext context, Persistable obj) throws MException {
        ...
        if (obj instanceof FoundationRelated) {
            ...
            if (f == null) return Ace.RIGHTS_NONE;
            {
                ...
                if (aclObject != null) return aclObject.getList();
            }
            {
                SopAcl aclObject =
                        M.l(AdbApi.class)
                                .getManager()
                                .getObjectByQualification(
                                        Db.query(SopAcl.class).eq("target", fId + "_"));
                if (aclObject != null) return aclObject.getList();
            }
