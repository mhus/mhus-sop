...
public class SopDataCmd extends AbstractCmd {
    ...
    @Option(
            name = "-r",
            aliases = "--r",
            description = "Use RAW Database commands",
            required = false)
    boolean raw = false;
    ...
    @Option(
            name = "-a",
            aliases = "--all",
            description = "For mass updates/list, update/list also archived objects",
            required = false)
    boolean all = false;
    ...
    @Option(
            name = "-s",
            aliases = "--sync",
            description = "Use build in sync mechanism while loading dataobject",
            required = false)
    boolean sync = false;
    ...
    public Object execute2() throws Exception {
        ...
        if (cmd.equals("list")) {
            ...
            if (found == null) throw new MException("Foundation not found", params[0]);
            ...
            for (int i = 1; i < params.length; i++) {
                ...
                if (key.equals("_foreignid")) {
                    d.setForeignId(val);
                } else if (key.equals("_status")) {
                    d.setStatus(val);
                } else if (key.equals("_due")) {
                    d.setDue(MDate.toDate(val, null));
                } else if (key.equals("_value0")) {
                    d.setValue0(val);
                } else if (key.equals("_value1")) {
                    d.setValue1(val);
                } else if (key.equals("_value2")) {
                    d.setValue2(val);
                } else if (key.equals("_value3")) {
                    d.setValue3(val);
                } else if (key.equals("_value4")) {
                    d.setValue4(val);
                } else if (key.equals("_value5")) {
                    d.setValue5(val);
                } else if (key.equals("_value6")) {
                    d.setValue6(val);
                } else if (key.equals("_value7")) {
                    d.setValue7(val);
                } else if (key.equals("_value8")) {
                    d.setValue8(val);
                } else if (key.equals("_value9")) {
                    d.setValue9(val);
                } else {
            ...
            for (int i = 2; i < params.length; i++) {
                ...
                if (pair.equals("public")) {
                    d.setPublic(true);
                } else if (pair.equals("rw")) {
                    d.setWritable(true);
                } else if (pair.equals("private")) {
                    d.setPublic(false);
                } else if (pair.equals("ro")) {
                    d.setWritable(false);
                } else {
                    ...
                    if (key.equals("_foreignid")) {
                        ...
                        d.setValue9(val);
                    } else if (key.equals("_writable")) {
                        d.setWritable(MCast.toboolean(val, false));
                    } else if (key.equals("_public")) {
            ...
            if (raw) d.save();
            else xcontroller.createSopData(d);
            ...
            if (params.length == 0) {
            } else if (MValidator.isUUID(params[0])) {
                SopData d = api.getSopData(null, params[0], sync);
                SopDataController xcontroller = api.getDataSyncControllerForType(d.getType());
                if (xcontroller != null) {
                    xcontroller.updateSopData(d);
                    System.out.println("UPDATE " + d);
                } else System.out.println("xcontroller not found: " + d);
            } else {
                SopFoundation found = api.getFoundation(params[0]);
        ...
        } else if (cmd.equals("syncListBeforeLoad")) {
            ...
            Boolean archived =
                    params.length > 3 && !params[3].equals("?")
                            ? MCast.toboolean(params[3], false)
                            : null;
            Date due =
                    params.length > 4 && !params[4].equals("?")
                            ? MCast.toDate(params[4], null)
                            : null;
            ...
            List<SopData> list =
                    api.getSopData(
                            found.getId(), type, search, true, archived, due, order, size, 0);
