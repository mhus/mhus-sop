...
public class RegistryMutex implements RegistryPathControl {
    ...
    public RegistryValue checkSetParameter(RegistryManager manager, RegistryValue value) {
        ...
        if (value.getPath().endsWith(RegistryUtil.MASTER_VARNAME)) {
            ...
            if (cur != null) mySeed = MCast.tolong(cur.getValue(), Long.MIN_VALUE);
            fireLock(LockListener.EVENT.LOCK, value, true);
            return new RegistryValue(
                    String.valueOf(mySeed),
                    value.getSource(),
                    value.getUpdated(),
                    value.getPath(),
                    Math.max(60000, value.getTimeout()),
                    false,
                    false);
        }
        return value;
    ...
    private void fireLock(LockListener.EVENT event, RegistryValue value, boolean local) {
        // this event is not asynchrony to have it synchrony to the end of the lock - fire before
    ...
    public RegistryValue checkSetParameterFromRemote(RegistryManager manager, RegistryValue value) {
        ...
        if (value.getPath().endsWith(RegistryUtil.MASTER_VARNAME)) {
            ...
            if (cur == null) {
                ...
                //				    working.set("");
                //				    try {
                //				        manager.setParameter(value.getPath(), String.valueOf(mySeed),
                // value.getTimeout(), false, false, false);
                //				    } finally {
                //				        working.remove();
                //                    }
                //					return null;
