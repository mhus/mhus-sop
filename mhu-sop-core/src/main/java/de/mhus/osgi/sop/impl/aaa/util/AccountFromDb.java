...
public class AccountFromDb extends MLog implements AccountSource, ModifyAccountApi {
    ...
    @Override
    public void createAccount(String username, String password, IReadProperties properties)
            throws MException {
    ...
    public void appendGroups(String username, String... groups) throws MException {
        Account acc = findAccount(username);
        if (acc == null) throw new MException("Account not found", username);
        for (String group : groups) ((SopAccount) acc).groups().add(group);
        ((SopAccount) acc).save();
    }
    ...
    public void removeGroups(String username, String... groups) throws MException {
        Account acc = findAccount(username);
        if (acc == null) throw new MException("Account not found", username);
        for (String group : groups) ((SopAccount) acc).groups().remove(group);
        ((SopAccount) acc).save();
    }
    ...
    public Collection<String> getAccountList(String filter) {
        ...
        if (filter.equals("*")) {
            ...
            if (not) query.not(Db.like("name", "%" + filter + "%"));
            else query.like("name", "%" + filter + "%");
        ...
        try {
            ...
            for (SopAccount entry : res) out.add(entry.getName());
        } catch (Exception e) {
            log().e(filter, e);
        }
        return out;
