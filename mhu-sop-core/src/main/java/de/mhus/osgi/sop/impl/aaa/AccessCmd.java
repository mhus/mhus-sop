...
public class AccessCmd extends AbstractCmd {
    ...
    @Option(
            name = "-a",
            aliases = "--admin",
            description = "Connect user as admin",
            required = false)
    boolean admin = false;
    ...
    public Object execute2() throws Exception {
        ...
        if (cmd.equals("validate")) {
            ...
            AaaContext cur =
                    api.process(
                            ...
                            null,
                            admin,
                            parameters.length > 1 ? Locale.forLanguageTag(parameters[1]) : null);
            ...
            else return api.hasGroupAccess(ac, parameters[1], null, null);
            ...
            } else if (parameters[0].equals("trust")) {
                try {
        ...
        } else if (cmd.equals("controllers")) {
            ...
            for (Entry<String, DbSchemaService> entry : adb.getController()) {
                ...
                table.addRowValues(
                        entry.getKey(), entry.getValue().getClass(), bundle.getSymbolicName());
            }
        ...
        } else if (cmd.equals("migrateFilesToCurrent")) {
            ...
            for (File file : dir.listFiles()) {
                ...
                try {
                    if (file.isFile() && file.getName().endsWith(".xml")) {
                        AccountFile from =
                                new AccountFile(file, MFile.getFileNameOnly(file.getName()));
                        ModifyAccountApi mod = api.getModifyAccountApi();
                        mod.createAccount(
                                from.getName(), UUID.randomUUID().toString(), from.getAttributes());
        ...
        } else System.out.println("Command not found: " + cmd);
