...
public class AdbApiImpl extends MLog implements AdbApi {
    ...
    @Override
    public SopActionTask createActionTask(
        ...
        if (smart) {
            SopActionTask t =
                    manager.getObjectByQualification(
                            Db.query(SopActionTask.class)
                                    .eq("queue", queue)
                                    .eq("action", action)
                                    .eq("target", target));
            if (t != null) return t;
    ...
    public List<SopActionTask> getQueue(String queue, int max) throws MException {
        ...
        DbCollection<SopActionTask> res =
                manager.getByQualification(
                        Db.query(SopActionTask.class).eq(_SopActionTask._QUEUE, queue));
    ...
    //	public ObjectParameter getRecursiveParameter(String type, UUID id, String key) throws
    // MException {
    //
    ...
    public SopObjectParameter getRecursiveParameter(DbMetadata obj, String key) throws MException {
        ...
        while (obj != null && level > 0) {
            ...
            SopObjectParameter out =
                    getParameter(obj.getClass().getCanonicalName(), obj.getId(), key);
    ...
    public void deleteAll(String type, UUID id) throws MException {
        ...
        for (SopObjectParameter p : getParameters(type, id)) if (canDelete(p)) p.delete();
    }
    ...
    public void deleteParameters(Class<?> type, UUID id) throws MException {
        for (SopObjectParameter p : getParameters(type, id)) p.delete();
    }

    @Override
    public List<SopObjectParameter> getParameters(Class<?> type, String key, String value)
            throws MException {
