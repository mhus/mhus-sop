...
public class RegisterUtil {
    ...
    public static boolean isEmpty(String name) throws MException {
        long cnt =
                M.l(SopApi.class).getManager().count(Db.query(SopRegister.class).eq("name", name));
        return cnt == 0;
    }
    ...
    public static void fillKeyValue(String name, Map<String, String> map) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            try {
                db.inject(new SopRegister(name, entry.getKey(), null, entry.getValue(), null))
                        .save();
            } catch (MException e) {
    ...
    public static void fillKeyKey(String name, Map<String, String> map) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            try {
                db.inject(new SopRegister(name, entry.getKey(), entry.getValue(), null, null))
                        .save();
            } catch (MException e) {
    ...
    public static boolean hasKey(String name, String key) throws MException {
        ...
        SopRegister out =
                db.getObjectByQualification(
                        Db.query(SopRegister.class).eq("name", name).eq("key1", key));
        return out != null;
    }
    ...
    public static SopRegister getByKeyKey(Class<?> clazz, String key1, String key2)
            throws MException {
    ...
    public static List<SopRegister> listByKey(String name, String key) throws MException {
        XdbService db = M.l(SopApi.class).getManager();
        if (key == null) return null;
        return db.getByQualification(Db.query(SopRegister.class).eq("name", name).eq("key1", key))
                .toCacheAndClose();
    }

    public static List<SopRegister> listByKeyKey(Class<?> clazz, String key1, String key2)
            throws MException {
        return listByKeyKey(clazz.getCanonicalName(), key1, key2);
    }

    public static List<SopRegister> listByKeyKey(String name, String key1, String key2)
            throws MException {
        XdbService db = M.l(SopApi.class).getManager();
        if (key1 == null || key2 == null) return null;
        return db.getByQualification(
                        Db.query(SopRegister.class)
                                .eq("name", name)
                                .eq("key1", key1)
                                .eq("key2", key2))
    ...
    public static SopRegister set(
            Class<?> clazz, String key1, String key2, String value1, String value2)
            throws MException {
        return set(clazz.getCanonicalName(), key1, key2, value1, value2);
    }

    public static SopRegister set(
            String name, String key1, String key2, String value1, String value2) throws MException {
